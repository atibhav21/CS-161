...
(defun at-least-one-color-helper (n c k sofar)
  (cond 
    ((> c k) sofar)
    (T (at-least-one-color-helper n (+ c 1) k (append (list (node2var n c k)) sofar)))
  )
)
...
(defun at-least-one-color (n c k)
  ...
  ;)
  (at-least-one-color-helper n c k NIL)
)

; negates all the literals in the clause and returns it as a single clause
(defun negate (l sofar)
  (cond 
      ((null L) sofar)
      (T (negate (rest l) (append sofar (list (- (first l))) ) ))
      ;(T (cons (- (first l)) (negate (rest l))))
  )
)
...
(defun pair-up-first (n c k_max k sofar)
  (cond 
    ((= c k) sofar)
    (T (pair-up-first n c k_max (- k 1) (append (list (negate (list (node2var n c k_max) (node2var n k k_max)) NIL)) sofar)))
    ;(T (append (pair-up-first c (- k 1)) (list (negate (list c k)))))
  )
)

; to make the function tail recursive
(defun at-most-one-color-helper (n c k sofar)
  (cond
    ((> c k) sofar)
    (T (at-most-one-color-helper n (+ c 1) k (append sofar (pair-up-first n c k k NIL))));(node2var n c k) k NIL))))
  )
)
...
(defun at-most-one-color (n c k)
  ;(cond 
   ; ((> c k) NIL)
    ;(T (append (pair-up-first (node2var n c k) k) (at-most-one-color n (+ c 1) k)))
  ;)
  (at-most-one-color-helper n c k NIL)
)
...
(defun generate-node-clauses (n k)
  ; call at-most-one-color and at-least-one-color but need to figure out the value of c to call with 
  (append (list (at-least-one-color n 1 k)) (at-most-one-color n 1 k)) ; works but causes stack overflow
)

; takes in 2 nodes and generates the clause that n1 and n2 are not the same color, c
; returns * a list of clauses *
(defun color-clause (n1 n2 c k)
...
(defun generate-edge-clauses-helper (n1 n2 c k sofar)
  (cond 
    ((> c k) sofar)
    (T (generate-edge-clauses-helper n1 n2 (+ c 1) k (append sofar (color-clause n1 n2 c k)) ))
  )
)
